...
- **技术动态** — GitHub Trending 热榜、AI 资讯（OpenAI / DeepMind / Hugging Face / TLDR AI / Smol AI / Latent Space / MIT Tech Review）、X 推文（attentionvc-ai）
...
- **社区讨论** — zh 模式 V2EX / LinuxDo 中文社区，en 模式 Hacker News / r/stocks 英文社区

报告以 `daily_reports/<日期>/<日期>.html` 落盘（每天一个子目录）。日期默认使用系统本地时区，可用 `REPORT_TZ` 覆盖（如 `Asia/Shanghai` / `UTC`）。HTML 单文件，CSS+JS 全内联，无外部依赖。

## 设计要点

- **部署灵活**：GitHub Actions + Pages（零基础设施）、本地一键装（用系统调度器 Windows Task Scheduler / macOS launchd / Linux cron）、自托管服务器，任选或并存
- **不绑定任何第三方阅读服务**：Feedly / Inoreader / Pocket 那些都会记录你看了什么；本项目不跟它们任何一个通信。数据源直接走公开 RSS / JSON 端点拉取
- **LLM 后端可插拔**：通过 `LLM_BACKEND` 环境变量在 claude CLI / Anthropic / OpenAI / DeepSeek / MiniMax 之间一行切换。默认 `claude-cli` 复用 Claude Code 已有的认证（不需要 API key），其他后端按各家 key 计费——见 [LLM 后端配置](#llm-后端配置)
...
- `["en"]` — **仅英文 mode**（Hacker News / r/stocks 等英文社区源，用于替代中文社区源）。zh mode 不显示
...
英文 mode 完整切换：HTML UI 文案、enrichment / digest / trading-commentary 三套 prompt、stance 词（"偏上行/偏下行/中性" → Bullish/Bearish/Neutral）、日期格式（zh-CN → en-GB）、Markdown 输出 — 全部跟着 `REPORT_LOCALE` 切。**中文社区源在英文 mode 下被自动过滤掉**（没 fork 用户读英文却想看 V2EX 帖子的合理场景）。
